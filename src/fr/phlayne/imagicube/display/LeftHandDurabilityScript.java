...
public class LeftHandDurabilityScript extends DisplayScript {

	private double order = 5;
	...
	public SimpleJSON getMessage(Player player) {
		...
		float durability = -1;
		if (new NBTItem(player.getEquipment().getItem(leftHanded ? EquipmentSlot.HAND : EquipmentSlot.OFF_HAND))
				.hasKey(NBTUtil.DURABILITY))
			durability = Durability.getPercentDurability(new NBTItem(
					player.getEquipment().getItem(leftHanded ? EquipmentSlot.HAND : EquipmentSlot.OFF_HAND)));
		if (durability != -1)
	...
	public boolean shouldDisplay(Player player) {
		boolean leftHanded = player.getMainHand().equals(MainHand.LEFT);
		ItemStack i = player.getEquipment().getItem(leftHanded ? EquipmentSlot.HAND : EquipmentSlot.OFF_HAND);
